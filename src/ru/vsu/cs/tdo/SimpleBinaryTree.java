...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    public T getAnswer(String bracketStr) throws Exception {
        this.fromBracketNotation(bracketStr);
        BinaryTreeAlgorithms.inOrderVisit(root, (T value, int level) -> {
            if (howMuchSuchThingsInTree(value) > maxCount) {
                maxCount = howMuchSuchThingsInTree(value);
                maxCountThing = value;
            }
        });
        return maxCountThing;
    }

    private int howMuchSuchThingsInTree(T value) {
        BinaryTreeAlgorithms.inOrderVisit(root, (T val, int level) -> {
            if (val.equals(value)) {
                currCount++;
            }
        });
